...
# ==========================================
# 🔍 SCANNER AUTOMATIQUE DE LOGOS (OPTIMISÉ CACHE BASE64)
# ==========================================
@st.cache_data
        ...
        for root, dirs, files in os.walk(dossier_racine):
            for file in files:
                if file.lower().endswith(('.png', '.jpg', '.jpeg')):
                    ...
                    try:
                        # On encode directement en Base64 au chargement initial
...
# --- OUTIL : FICHES DE MATCHS (AVEC PAGINATION & LOGOS INSTANTANÉS) ---
def afficher_resultats(df_resultats):
    ...
    if mode == "📊 Tableau classique":
        ...
        # --- SYSTÈME DE PAGINATION (Maximum 10 fiches par page) ---
        taille_page = 10
        total_matchs = len(df_resultats)
        
        if total_matchs > taille_page:
            nb_pages = ((total_matchs - 1) // taille_page) + 1
            col_vide, col_nav = st.columns([2, 1])
            with col_nav:
                page_selectionnee = st.selectbox("Page", range(1, nb_pages + 1), index=0, key="paginator")
            
            debut_idx = (page_selectionnee - 1) * taille_page
            fin_idx = min(debut_idx + taille_page, total_matchs)
            df_page = df_resultats.iloc[debut_idx:fin_idx]
            st.caption(f"Affichage des archives {debut_idx + 1} à {fin_idx} sur {total_matchs}")
        else:
        ...
        for i, (index, row) in enumerate(df_page.iterrows()):
            with cols[i % 2]:
                with st.container(border=True):
                    ...
                    # Récupération instantanée depuis la mémoire cache (plus de open() lent ici)
                    logo_dom_b64 = DICTIONNAIRE_LOGOS_EQUIPES.get(nettoyer_nom_equipe(dom))
                    logo_ext_b64 = DICTIONNAIRE_LOGOS_EQUIPES.get(nettoyer_nom_equipe(ext))
                    
                    c_dom, c_score, c_ext = st.columns([1, 1, 1])
                    
                    with c_dom:
                        if logo_dom_b64:
                            st.markdown(f"<div style='text-align:center;'><img src='data:image/png;base64,{logo_dom_b64}' style='width:60px; margin-bottom:5px;'></div>", unsafe_allow_html=True)
                    ...
                    with c_ext:
                        if logo_ext_b64:
                            st.markdown(f"<div style='text-align:center;'><img src='data:image/png;base64,{logo_ext_b64}' style='width:60px; margin-bottom:5px;'></div>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    /* Styles pour boutons d'action Accueil sécurisés (Plus de nth-child instables) */
...
if st.session_state.page == 'accueil':
    ...
    # Exemple d'application des balises cibles pour les gros boutons colorés du bas (si présents dans la suite du code)
    # st.markdown('<div class="btn-vert-cible" style="margin-bottom: -15px;"></div>', unsafe_allow_html=True)
    # st.button("Bouton Vert d'Action")
