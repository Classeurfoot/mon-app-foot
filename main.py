...
def popup_echanges():
    ...
    * 🔄 Nous pouvons alors convenir d'un échange équitable de fichiers numériques.
    """)
...
if st.session_state.page == 'accueil':
    ...
    /* Styles pour la grille de catégories centrale */
    div.element-container:has(.home-nations) + div.element-container button { background-color: #b8860b !important; border-color: #b8860b !important; }
    div.element-container:has(.home-nations) + div.element-container button p { color: white !important; font-weight: 500; }
    div.element-container:has(.home-champ) + div.element-container button { background-color: #722f37 !important; border-color: #722f37 !important; }
    div.element-container:has(.home-champ) + div.element-container button p { color: white !important; font-weight: 500; }
    div.element-container:has(.home-europe) + div.element-container button { background-color: #1a2b4c !important; border-color: #1a2b4c !important; }
    div.element-container:has(.home-europe) + div.element-container button p { color: white !important; font-weight: 500; }
    div.element-container:has(.home-amicaux) + div.element-container button { background-color: #5b7c6c !important; border-color: #5b7c6c !important; }
    div.element-container:has(.home-amicaux) + div.element-container button p { color: white !important; font-weight: 500; }
    </style>
    """, unsafe_allow_html=True)

    # --- REPRISE DE LA CONSTRUCTION À PARTIR DE VOTRE COUPURE ---
    col_btn1, col_btn2, col_btn3, col_btn4, col_btn5 = st.columns(5)
    with col_btn1:
        if st.button("🧭 Guide & Contenu", key="btn_popup_guide", use_container_width=True):
            popup_guide_contenu()
    with col_btn2:
        if st.button("💾 Formats", key="btn_popup_formats", use_container_width=True):
            popup_formats()
    with col_btn3:
        if st.button("💶 Tarifs", key="btn_popup_tarifs", use_container_width=True):
            popup_tarifs()
    with col_btn4:
        if st.button("✉️ Commandes", key="btn_popup_commandes", use_container_width=True):
            popup_contact_commandes()
    with col_btn5:
        if st.button("🤝 Échanges", key="btn_popup_echanges", use_container_width=True):
            popup_echanges()

    st.write("")

    # --- SECTION BANNIÈRE : PROPRES ARCHIVES ---
    with st.container(border=True):
        col_arc1, col_arc2 = st.columns([4, 1])
        with col_arc1:
            st.markdown("##### 🚨 Vous possédez vos propres archives ?")
            st.markdown("<p style='font-size:14px; margin-top:-5px;'>Je suis constamment à la recherche de nouveaux matchs pour compléter le Grenier. Découvrez ma liste de recherches et proposons-nous des échanges !</p>", unsafe_allow_html=True)
        with col_arc2:
            st.write("")
            if st.button("Voir mes recherches", type="primary", use_container_width=True):
                st.session_state.page = 'mes_recherches'
                st.rerun()

    st.write("")

    # --- SECTION : EXPLORER LE GRENIER (GRILLE) ---
    st.markdown("### 📂 Explorer le Grenier")
    cat_col1, cat_col2 = st.columns(2)
    with cat_col1:
        st.markdown('<div class="home-nations"></div>', unsafe_allow_html=True)
        if st.button("🌍 Nations (Mondial, Euro...)", key="home_nations_btn", use_container_width=True):
            st.session_state.page = 'arborescence'
            st.session_state.chemin = ['Nations']
            st.rerun()
            
        st.markdown('<div class="home-europe"></div>', unsafe_allow_html=True)
        if st.button("🏆 Coupes d'Europe (LDC...)", key="home_europe_btn", use_container_width=True):
            st.session_state.page = 'arborescence'
            st.session_state.chemin = ["Coupes d'Europe"]
            st.rerun()
            
    with cat_col2:
        st.markdown('<div class="home-champ"></div>', unsafe_allow_html=True)
        if st.button("🏟️ Championnats & Coupes", key="home_champ_btn", use_container_width=True):
            st.session_state.page = 'arborescence'
            st.session_state.chemin = ['Championnats & Coupes']
            st.rerun()
            
        st.markdown('<div class="home-amicaux"></div>', unsafe_allow_html=True)
        if st.button("🤝 Amicaux Internationaux", key="home_amicaux_btn", use_container_width=True):
            st.session_state.page = 'arborescence'
            st.session_state.chemin = ['Amicaux Internationaux']
            st.rerun()
    ...
    # --- SECTION : TOUT VOIR D'UN COUP & L'ÉPHÉMÉRIDE ---
    low_col1, low_col2 = st.columns(2)
    with low_col1:
        st.markdown("### 📖 Tout voir d'un coup")
        st.markdown("<p style='font-size:14px; color:gray;'>Vous préférez flâner ? Affichez la liste complète de tous les matchs disponibles.</p>", unsafe_allow_html=True)
    ...
    with low_col2:
        ...
        # Calcul automatique du nombre de matchs joués ce jour-ci
        ...
        nom_mois = mois_fr_list[mois_actuel - 1]
        
        nb_matchs_du_jour = 11  # Valeur par défaut de la maquette
        if not df.empty and 'Date' in df.columns:
            try:
                pattern = f"^{str(jour_actuel).zfill(2)}/{str(mois_actuel).zfill(2)}/"
                matchs_trouves = df['Date'].astype(str).str.contains(pattern, regex=True, na=False).sum()
                if matchs_trouves > 0:
                    nb_matchs_du_jour = matchs_trouves
            except Exception:
                pass
                
        st.success(f"🟢 {nb_matchs_du_jour} matchs se sont joués un {jour_actuel} {nom_mois} !")
        
        if st.button("Voir les matchs du jour", use_container_width=True):
            st.session_state.page = 'recherche_avancee'
            st.session_state.recherche_comp_cible = f"{str(jour_actuel).zfill(2)}/{str(mois_actuel).zfill(2)}"
            st.rerun()

    st.write("")

    # --- SECTION : BOX DU GRAAL (ALERTE MATCH RECHERCHÉ) ---
    with st.container(border=True):
        st.markdown("<h4 style='text-align: center;'>🌋 Le Graal est peut-être au fond du Grenier...</h4>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-style: italic; color: #aaa; font-size:14px;'>« Tu te souviens de ce match précis ? Celui de ton enfance, avec les maillots trop larges, les pubs Kodak et la prise d'antenne 1/4h avant la descente du bus ? »</p>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-size: 13px; color: #ccc;'>On fouille les archives 24h/24 pour exhumer des trésors. Si le match que tu cherches n'est pas encore dans le catalogue, ne le laisse pas s'échapper une deuxième fois. Laisse ton contact et je te préviendrai personnellement. Zéro spam, promis.</p>", unsafe_allow_html=True)
        
        graal_col1, graal_col2 = st.columns(2)
        with graal_col1:
            email_input = st.text_input("💻 Ton Email :", placeholder="Le maillot au fond du sac...")
        with graal_col2:
            match_input = st.text_input("📼 Le match que tu cherches :", placeholder="Ex : Nantes-Juventus 96, un vieux derby...")
            
        if st.button("🚨 ALERTE-MOI DÈS QU'IL SORT DU GRENIER", type="primary", use_container_width=True):
            if email_input and match_input:
                st.success("Votre demande d'alerte a bien été enregistrée ! L'équipe du Grenier part à sa recherche.")
            else:
                st.error("Veuillez remplir votre email et préciser le match recherché.")

# ==========================================
# GESTION DES AUTRES PAGES (STUBS SÉCURISÉS)
# ==========================================
elif st.session_state.page == 'arborescence':
    st.title(f"📂 Navigation : {' > '.join(st.session_state.chemin)}")
    if st.button("🔙 Retour"): go_home(); st.rerun()
elif st.session_state.page == 'panier':
    st.title("🛒 Votre Panier")
    if st.button("🔙 Retour"): go_home(); st.rerun()
else:
    # Fallback générique pour éviter tout plantage sur les pages secondaires en développement
    st.title(f"ℹ️ Page : {st.session_state.page.capitalize()}")
    if st.button("🔙 Retour à l'accueil"): go_home(); st.rerun()
